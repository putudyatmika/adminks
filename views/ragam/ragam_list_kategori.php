					...
					if ($r_kat["error"]==false) {
						...
						for ($i=1;$i<=$max_kat;$i++) {
							...
							<tr>
								...
								<td><div class="text-center"><a href="'.$app_url.'/'.$page.'/editkat/'.$lvl3.'/'.$r_kat["item"][$i]["tema_kat_id"].'" class="btn btn-xs btn-success">Edit</a> 
								<a href="'.$app_url.'/'.$page.'/hapuskat/'.$lvl3.'/'.$r_kat["item"][$i]["tema_kat_id"].'" class="btn btn-xs btn-danger" data-confirm="Apakah data kategori ini akan di hapus?">Del</a></div></td>
							</tr>
						';	
						}
